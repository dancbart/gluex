...
// SMALL FILE (same dataset, but only flattened 3-4 runs)
...
void plots_KSpipLamb(bool bggen=false){
  ...
  c1->Divide(3,2);
  c1->cd(1);
  TH1F* hM1 = FSModeHistogram::getTH1F(FND,NT,"m100000000_1100","EnUnusedSh","(100,0.0,1.0)","CUT(tRange,rf,chi2DOF,unusedE,unusedTracks,coherentPeak)");
  hM1->SetTitle("E_{unused} for -t in (0.0,1.0)");
