...
void setup(){
  ...
  DEFAULT_CUTS = "eBeam,chi2,rf,unusedE,unusedTracks,z,MM2,t,Lambda,Pi0,KpiMass"; //chi2rank
  DEFAULT_CUTS += ",Sigma1385Veto";
}

void plot_pi0kplamb(bool bggen=false){
  ...
  TString FND_DATA = "tree_pi0kplamb__B4_M7_M18_BestChi2_SKIM*.root";
  TString FND_BGGEN = "tree_pi0kplamb__B4_M7_M18_BGGEN_BestChi2_SKIM*.root";
  TString FND_MC = "tree_pi0kplamb__B4_M7_M18_PSMC_BestChi2_SKIM*.root"; 
  ...
  cb->Divide(3,2);
    ...
    TH1F* hEnUnusedSh = FSModeHistogram::getTH1F(FND_DATA,NT,"pi0kplamb","EnUnusedSh","(100,0.0,1.0)",Form("CUT(%s)",CUTS.Data()));
    hEnUnusedSh->SetXTitle("E_{unused}  [GeV/c^{2}]");
    hEnUnusedSh->SetYTitle("Events");
    hEnUnusedSh->Draw();
    TH1F* hEnUnusedShMC = FSModeHistogram::getTH1F(FND_MC,NT,"pi0kplamb","EnUnusedSh","(100,0.0,1.0)",Form("CUT(%s)",CUTS.Data()));
    ...
    CUTS = DEFAULT_CUTS;
    CUTS.ReplaceAll(",z","");
    TH1F* hProdVz = FSModeHistogram::getTH1F(FND_DATA,NT,"pi0kplamb","ProdVz","(100,0.,100.0)",Form("CUT(%s)",CUTS.Data()));
    hProdVz->SetXTitle("ProdVz  [cm]");
    hProdVz->SetYTitle("Events");
    hProdVz->Draw();
    TH1F* hProdVzMC = FSModeHistogram::getTH1F(FND_MC,NT,"pi0kplamb","ProdVz","(100,0.,100.0)",Form("CUT(%s)",CUTS.Data()));
    ...
    CUTS = DEFAULT_CUTS;
    CUTS.ReplaceAll(",t","");
    TH1F* htk = FSModeHistogram::getTH1F(FND_DATA,NT,"pi0kplamb","abs(-1*MASS2(1,-GLUEXTARGET))","(100,0,5)",Form("CUT(%s)",CUTS.Data()));
    htk->SetXTitle("|-t| [GeV^{2}]");
    htk->SetYTitle("Entries");
    htk->Draw();
    htk->Fit("expo","","",0.5,1.0);
    TH1F* htkMC = FSModeHistogram::getTH1F(FND_MC,NT,"pi0kplamb","abs(-1*MASS2(1,-GLUEXTARGET))","(100,0,5)",Form("CUT(%s)",CUTS.Data()));
    ...
    TH1F* hEnPB = FSModeHistogram::getTH1F(FND_DATA,NT,"pi0kplamb","EnPB","(125,5,12)",Form("CUT(%s)",CUTS.Data()));
    hEnPB->SetXTitle("E_{beam} [GeV]");
    hEnPB->SetYTitle("Entries");
    hEnPB->Draw();
    TH1F* hEnPBMC = FSModeHistogram::getTH1F(FND_MC,NT,"pi0kplamb","EnPB","(125,5,12)",Form("CUT(%s)",CUTS.Data()));
    ...
    CUTS = DEFAULT_CUTS;
    CUTS.ReplaceAll(",MM2","");
    TH1F* hMM2 = FSModeHistogram::getTH1F(FND_DATA,NT,"pi0kplamb","RMASS2(GLUEXTARGET,B,-1,-2,-[pi0])","(100,-0.1,0.1)",Form("CUT(%s)",CUTS.Data()));
    hMM2->SetXTitle("Missing Mass Squared [GeV^{2}]");
    hMM2->SetYTitle("Entries");
    hMM2->Draw();
    TH1F* hMM2MC = FSModeHistogram::getTH1F(FND_MC,NT,"pi0kplamb","RMASS2(GLUEXTARGET,B,-1,-2,-[pi0])","(100,-0.1,0.1)",Form("CUT(%s)",CUTS.Data()));
    ...
    TH1F* hChi2DOF = FSModeHistogram::getTH1F(FND_DATA,NT,"pi0kplamb","Chi2DOF","(40,0,20)",Form("CUT(%s)",CUTS.Data()));
    hChi2DOF->SetXTitle("#chi^{2}/dof");
    hChi2DOF->SetYTitle("Events");
    hChi2DOF->Draw();
    TH1F* hChi2DOFMC = FSModeHistogram::getTH1F(FND_MC,NT,"pi0kplamb","Chi2DOF","(40,0,20)",Form("CUT(%s)",CUTS.Data()));
    ...
    TH1F* hChi2DOFSB = FSModeHistogram::getTH1F(FND_DATA,NT,"pi0kplamb","Chi2DOF","(40,0,20)",Form("CUT(%s)",CUTS.Data()));
    ...
    c1->Print("plots/pi0kplamb_cuts.pdf");

    if(bggen) {
	    cb->cd(1);   
	    CUTS = DEFAULT_CUTS;
	    CUTS.ReplaceAll(",chi2,",",");
	    TH1F* hChi2DOF_BGGEN = FSModeHistogram::getTH1F(FND_BGGEN,NT,"pi0kplamb","Chi2DOF","(40,0,20)",Form("CUT(%s)",CUTS.Data()));
	    hChi2DOF_BGGEN->SetXTitle("#chi^{2}/dof");
	    hChi2DOF_BGGEN->SetYTitle("Events");
	    hChi2DOF_BGGEN->Draw();
	    FSModeHistogram::drawMCComponentsSame(FND_BGGEN,NT,"pi0kplamb","Chi2DOF","(40,0,20)",Form("CUT(%s)",CUTS.Data()));
    }

    // Momentum and angle distributions
    CUTS = "eBeam,rf,Lambda,Pi0"; //DEFAULT_CUTS;
    ...
    TH2F* hLambdaMomentumVsPathLengthSignal = FSModeHistogram::getTH2F(FND_DATA,NT,"pi0kplamb","RMOMENTUM(1):VeeLP1","(100,-50,50,100,0,1)",Form("CUT(%s)",CUTS.Data()));
    TH2F* hLambdaMomentumVsPathLengthMC = FSModeHistogram::getTH2F(FND_MC,NT,"pi0kplamb","RMOMENTUM(1):VeeLP1","(100,-50,50,100,0,1)",Form("CUT(%s)",CUTS.Data()));

    TH2F* hProtonMomentumVsThetaSignal = FSModeHistogram::getTH2F(FND_DATA,NT,"pi0kplamb","RMOMENTUM(1a):acos(RCOSINE(1a))*180/3.141","(180,0,90,100,0,1)",Form("CUT(%s)",CUTS.Data()));
    TH2F* hProtonMomentumVsThetaMC = FSModeHistogram::getTH2F(FND_MC,NT,"pi0kplamb","RMOMENTUM(1a):acos(RCOSINE(1a))*180/3.141","(180,0,90,100,0,1)",Form("CUT(%s)",CUTS.Data()));
    TH2F* hPiMinusMomentumVsThetaSignal = FSModeHistogram::getTH2F(FND_DATA,NT,"pi0kplamb","RMOMENTUM(1b):acos(RCOSINE(1b))*180/3.141","(180,0,90,100,0,0.5)",Form("CUT(%s)",CUTS.Data()));
    TH2F* hPiMinusMomentumVsThetaMC = FSModeHistogram::getTH2F(FND_MC,NT,"pi0kplamb","RMOMENTUM(1b):acos(RCOSINE(1b))*180/3.141","(180,0,90,100,0,0.5)",Form("CUT(%s)",CUTS.Data()));

    TH2F* hProtonMomentumVsPiMinusMomentumSignal = FSModeHistogram::getTH2F(FND_DATA,NT,"pi0kplamb","RMOMENTUM(1a):RMOMENTUM(1b)","(100,0,0.5,100,0,1)",Form("CUT(%s)",CUTS.Data()));
    TH2F* hProtonMomentumVsPiMinusMomentumMC = FSModeHistogram::getTH2F(FND_MC,NT,"pi0kplamb","RMOMENTUM(1a):RMOMENTUM(1b)","(100,0,0.5,100,0,1)",Form("CUT(%s)",CUTS.Data()));

    TH2F* hProtonThetaVsPiMinusThetaSignal = FSModeHistogram::getTH2F(FND_DATA,NT,"pi0kplamb","acos(COSINE(1a))*180/3.141:acos(COSINE(1b))*180/3.141","(100,0,90,100,0,90)",Form("CUT(%s)",CUTS.Data()));
    TH2F* hProtonThetaVsPiMinusThetaMC = FSModeHistogram::getTH2F(FND_MC,NT,"pi0kplamb","acos(COSINE(1a))*180/3.141:acos(COSINE(1b))*180/3.141","(100,0,90,100,0,90)",Form("CUT(%s)",CUTS.Data()));
    ...
    CUTS.ReplaceAll(",Pi0","");
    CUTS.ReplaceAll(",Lambda","");
    TH1F* hMkppi02DSB = FSModeHistogram::getTH1F(FND_DATA,NT,"pi0kplamb","MASS([K+],[pi0])","(100,0.5,2.0)",Form("CUT(%s)*CUTWT(Lambda2DSB,Pi02DSB)",CUTS.Data()));
    ...
    if(bggen) cb->Print("plots/pi0kplamb_bggen.pdf");
    ...
    c11->Print("plots/pi0kplamb_mass_sb.pdf");

    c22->Print("plots/pi0kplamb_mass.pdf");
