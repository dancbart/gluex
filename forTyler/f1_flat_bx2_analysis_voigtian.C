...
void f1_flat_bx2_analysis_voigtian() {
   ...
   // ********** DEFINITIONS **********
    ...
    // Create a Voigtian
    std::unique_ptr<RooVoigtian> voigt = std::make_unique<RooVoigtian>("voigt", "RooVoigtian", x, mean, width, sigma);

    // Combine functions: //

    std::unique_ptr<TF1> fitCombined = std::make_unique<TF1>("fitCombined", "voigt", 1.2, 1.7);
    ...
    legend1->AddEntry(fitCombined.get(), "RooVoigtian", "l");
    legend1->Draw();
